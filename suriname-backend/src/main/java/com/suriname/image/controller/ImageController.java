...
public class ImageController {
    ...
    public ResponseEntity<Map<String, Object>> uploadImage(
        ...
        try {
            ...
            log.error("파일 I/O 업로드 실패: requestId={}, file={}, error={}", requestId, file.getOriginalFilename(), e.getMessage());
            return ResponseEntity.badRequest().body(Map.of(
                    "status", 400,
                    "message", "파일 업로드에 실패했습니다: " + e.getMessage()
            ));
        } catch (Exception e) {
            log.error("이미지 업로드 실패: requestId={}, file={}, error={}", requestId, file.getOriginalFilename(), e.getMessage());
            return ResponseEntity.badRequest().body(Map.of(
                    "status", 400,
