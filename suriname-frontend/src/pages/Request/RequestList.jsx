...
export default function RequestList() {
  ...
  const STATUS_DEFAULT = { priority: 99, color: "#6b7280", label: "미정" };
  const getStatusMeta = (s) => statusConfig?.[s] ?? STATUS_DEFAULT;
  ...
  const fetchRequests = async (searchParams = null) => {
    ...
    try {
      ...
      const normalized = content.map((it) => ({
        ...it,
        assignmentStatus:
          it?.assignmentStatus ?? it?.status ?? it?.requestStatus ?? "UNKNOWN",
      }));

      console.log(
        "assignmentStatus 목록:",
        normalized.map((x) => x.assignmentStatus)
      );

      normalized.sort(
        (a, b) =>
          getStatusMeta(a.assignmentStatus).priority -
          getStatusMeta(b.assignmentStatus).priority
      );
  ...
  const deleteSelectedRequests = async () => {
    ...
    if (
      !confirm(`선택된 ${selectedRequests.length}개 항목을 삭제하시겠습니까?`)
    ) {
      return;
    }
  ...
  const handleAccept = (request) => {
    ...
    if (!confirm(`[${request.requestNo}]를 접수(ACCEPTED) 상태로 변경할까요?`))
      return;
              ...
              {requests.length === 0 ? (
                ...
                requests.map((request) => (
                  ...
                  >
                    ...
                    {role === "ENGINEER" && (
                      <td>
                        <button
                          ...
                          disabled={
                            getStatusMeta(request.assignmentStatus) ===
                              STATUS_DEFAULT ||
                    ...
                    <td
                      style={{
                        color: getStatusMeta(request.assignmentStatus).color,
                      }}
                    >
                      {getStatusMeta(request.assignmentStatus).label}+{" "}
                    </td>
